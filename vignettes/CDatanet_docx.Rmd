...
```
## Estimation
Using the simulated data, I estimate the count data model as well as the SART and SAR models. The output of these models has a `summary class` to summarize the results. In addition, the `summary` function computes marginal effects for the count data and the SART models.
...
In contrast, the peer effect is overestimated when the network is assumed exogenous. The average of the estimates is 0.52. 
