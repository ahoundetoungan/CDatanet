...
urlcolor: blue
vignette: >
  %\VignetteIndexEntry{CDatanet: An  R package to simulate and estimate a Count Data Model with Social Interactions}
...
To simulate the network matrix, I assume that the number of friends of each individual is randomly chosen between 0 et 30.
...
In this section, I conduct Monte Carlo simulations to assess the performance of the estimator. I use the same settings as in Section [4.1](#exo.data) exepted that the number of individuals in each sub-network is set to 500. Let's build the following Monte Carlo function that simulates the data, estimates the three models and returns their estimators.
...
```
The stardard errors are slightly greater than the previous estimate. 
...
I build the following Monte Carlo function which simulates data based on endegenous netwok, and estimates the model by assuming that the network is exogenous and then endogenous.
