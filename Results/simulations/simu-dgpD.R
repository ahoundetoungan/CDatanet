############### THIS CODE REPLICATES THE MONTE CARLO RESULTS FOR DGP D ############### 
rm(list = ls())
library(CDatanet)
library(doParallel)
...
f.estim  <- function(nvec){
  ...
  while(cont){
    ...
    ecd[[Rbh]]  <- cdnet(y ~ x1 + x2 + gx1 + gx2, Glist = Gmu, group = grp, optimizer = "optim", 
                         npl.ctr = npl.ctr, opt.ctr = opt.ctr1, Rbar = rep(Rbh, 2), 
                         data = data, cov = FALSE, starting = starting, Ey0 = Ey0, Rmax = 100)
...
save(out500, out2000, file = "dgpD.rda")
write.csv(cbind(t(apply(do.call(cbind, out500), 1, sum.func)), 
                t(apply(do.call(cbind, out2000), 1, sum.func))), file = "dgpD.csv")
